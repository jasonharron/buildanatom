...
let text = createText("", 0.15, 0xffffff);
let textIon = createText("", 0.15, 0xffffff);
let textStable = createText("", 0.15, 0xffffff);
let textDebug = createText("", 0.15, 0xffffff);
let debug = new THREE.Vector3();
let stableValue = true;
let corePosition = 0;
let textProtons = createText("Protons: " + protonCount, 0.1, 0xff0000);
let textNeutrons = createText(
  "Neutrons: " + neutronCount,
  0.1,
  neutronTextColor
);
let textElectrons = createText("Electrons: " + electronCount, 0.1, 0x0000ff);
...
const core5 = [
  ...
  new THREE.Vector3(0.095, 1.6, fixedZ - 0.13), //this position can be improved
  new THREE.Vector3(-0.09, 1.58, fixedZ - 0.13), //this position can be improved
];
...
function init() {
  ...
  if (showIon) {
    textIon = createText("", 0.15, 0xffffff);
    textIon.position.set(1, 2.4, -1.5);
    scene.add(textIon);
  }

  if (showStable) {
    textStable = createText("", 0.15, 0xffffff);
    textStable.position.set(1, 2.4, -1.5);
    scene.add(textStable);
  }

  if (showDebug) {
    textDebug = createText("", 0.15, 0xffffff);
  ...
  if (showSymbol) {
    textSymbolLetter = createText("", 0.15, 0xffffff);
  ...
  for (let i = 0; i < 35; i++) {
    ...
    if (i % 3 == 0) {
      ...
      const innerSphereGeometry = new THREE.SphereGeometry(0.05);
      ...
      innerSphere.scale.set(0.5, 0.5, 0.5);
      object.add(innerSphere);
    }
  ...
  const resetButton = makeButtonMesh(0.4, 0.2, 0.01, 0x355c7d);
  const resetButtonText = createText("Reset", 0.1);
  resetButton.add(resetButtonText);
  resetButtonText.position.set(0, 0, 0.006);
  resetButton.position.set(0.8, 1.1, -1.5);
  resetButton.name = "reset";
    resetButtonText.name = "reset";
  ...
  for (let i = 0; i <= segments2; i++) {
    ...
    points2.push(Math.cos(theta2) * radius2, Math.sin(theta2) * radius2, 0);
  }
...
export function resetFunction() {
  for (let i = 0; i < group.children.length; i++) {
    if (group.children[i].name !== "reset") {
      group.children[i].position.x = group.children[i].homeX;
      group.children[i].position.y = group.children[i].homeY;
      group.children[i].position.z = group.children[i].homeZ;
    }
  }
}
    ...
    if (!positionOccupied[0] && !positionOccupied[1]) {
      ...
      for (let j = 2; j < positions.length; j++) {
        if (!positionOccupied[j]) {
          const distance = group.children[i].position.distanceTo(positions[j]);
...
function updateText() {
    ...
    if (showElement) {
      ...
      if (elementName === "X") {
        ...
        if (elementTextType == 0) {
          ...
          text.position.set(0.82, textHeight - 0.25, -1.495);
        }
      }
      scene.add(text);
    }
  ...
  if (protonCount !== oldProtonCount || electronCount !== oldElectronCount) {
    if (showIon) {
  ...
  if (protonCount !== oldProtonCount || neutronCount !== oldNeutronCount) {
    if (showStable) {
  ...
  if (
    ...
    textElectrons = createText("Electrons: " + electronCount, 0.1, 0x0000ff);
    textElectrons.position.set(-0.735, 1.9, -1.5);
    scene.add(textElectrons);
    ...
    if (showSymbol) {
      ...
      textSymbolProtons.position.set(0.66, 1.35, -1.5);
      scene.add(textSymbolProtons);
      textSymbolWeight = createText(protonCount + neutronCount, 0.12, 0x000000);
      textSymbolWeight.position.set(0.66, 1.75, -1.5);
      scene.add(textSymbolWeight);
      if (ionCount === 0) {
        ...
        textSymbolCharge = createText(Math.abs(ionCount) + "-", 0.12, 0x0000ff);
      }
      textSymbolCharge.position.set(0.95, 1.75, -1.5);
      scene.add(textSymbolCharge);
    }
...
function updateCameraPosition() {
  ...
  // Adjust camera position based on width ranges
  if (width < 600) {
    camera.position.z = 2.5; // Closer for small screens
  } else if (width < 750) {
    camera.position.z = 1.5; // Medium distance for tablets
  } else {
    camera.position.z = 0.75; // Farther for large screens
  }
}
